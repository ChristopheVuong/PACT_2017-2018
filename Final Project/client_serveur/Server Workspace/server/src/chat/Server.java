...
public final class Server {
	...
	public Server() {
		...
		try {
			...
			serverSocket = new ServerSocket(12292);
			start();
	...
	/**
	 * Permet d'envoyer un message à au client
	 */
									...
									if(ValidationMethods.isMoveInitializer(message) && !isReadingMove) {
										...
										if(isReadingMove) {
											if(ValidationMethods.isMoveEnd(message)) {
												final File fileMove = new File(Constants.BASE_MOVE + indexEdit.get(indexMove++).getFileName());
	...
	public void sendImage(final byte[] colorFrame) {
		...
		try {
			...
			(bais = new ByteArrayInputStream(baos.toByteArray())).read(bufferByte);
			sendMessageToClient("image=>imageInit");
			sendMessageToClient(new String(Base64.getEncoder().encode(bufferByte),StandardCharsets.UTF_8));
			sendMessageToClient("image=>imageEnd");
		} catch(final IOException e) {}
		finally {
			if(baos != null) {
				try {
